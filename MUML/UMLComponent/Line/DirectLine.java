...
public class DirectLine implements UMLShape {
	...
	// whether this point contain in
	public boolean containsPoint(Point p) {
		
		// check all points
		for (int i = 0; i < points.size() - 1; i++) {
			Point s = points.get(i);
			Point e = points.get(i + 1); // the next point
	...
	protected void adjustByPortInterface() {
		...
		Point a, b;
		a = this.points.get(0); // the start point of line
		a.setLocation(s);
		b = this.points.get(this.points.size() - 1); // the end point of line
		b.setLocation(e);
	}

	protected void adjustByAutoline() {
		// remove staggered points
	...
	protected void adjustByHighlight() {
		...
		if (this.displayType == LINE_HIGHLIGHT) {
			...
			if (this.startPortInterface.getUMLShapeContainer().getSelected() == false && 
					this.endPortInterface.getUMLShapeContainer().getSelected() == false) {
				this.displayType = LINE_NORMAL;
			}
		}
	}
